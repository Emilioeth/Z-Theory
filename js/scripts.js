...
---------------------------------------------------*/	
	...
	function PageLoadActions() {
		...
		// Default Page Navigation Load Events wowowow
		...
		if (!isMobile()) {			
			$(".getin-touch").on('mouseenter', function() {	
				var $this = $(this);			
				gsap.to('#ball', {duration: 0.3, borderWidth: '2px', scale: 1.2, borderColor:$("body").data('primary-color'), backgroundColor:$("body").data('primary-color')});
				gsap.to('#ball-loader', {duration: 0.2, borderWidth: '2px', top: 2, left: 2});
				$( "#ball" ).append( '<p class="first">' + $this.data("firstline") + '</p>' + '<p>' + $this.data("secondline") + '</p>' );				
			});								
			$(".getin-touch").on('mouseleave', function() {					
				gsap.to('#ball', {duration: 0.2, borderWidth: '4px', scale:0.5, borderColor:'#999999', backgroundColor:'transparent'});
				gsap.to('#ball-loader', {duration: 0.2, borderWidth: '4px', top: 0, left: 0});
				$('#ball p').remove();				
			});		
		...
		if( $('.portfolio-wrap').length > 0 ){
			...
			if (!isMobile()) {
				...
				function onMouseHoverLatest() {
					...
					text.innerHTML = 'LATEST PROJECT • LATEST PROJECT • LATEST PROJECT •'
				...
				const $hoverableLatest = document.querySelectorAll('.item-parallax');

				for (let i = 0; i < $hoverableLatest.length; i++) {
